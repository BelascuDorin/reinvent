 ...
 * decide who is worth opening — no bio or experience, which belong to the full public
 * profile. Only discoverable Mentors are ever rendered as one, so there is no
 * completeness flag here: a summary existing already means the Mentor is discoverable.
 * The Mentor's User id is the stable identifier a client uses to open their profile.
 */
